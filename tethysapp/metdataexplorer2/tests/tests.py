...
# from ..app import Metdataexplorer2
...
To run any tests:
    1. Open a terminal
    2. Enter command ". /usr/lib/tethys/bin/activate" to activate the Tethys python environment
    3. In portal_config.yml make sure that the default database user is set to tethys_super
    or is a super user of the database
        DATABASES:
            default:
    ...
    4. Enter tethys test command.
       The general form is: "tethys test -f tethys_apps.tethysapp.<app_name>.<folder_name>.
       <file_name>.<class_name>.<function_name>"
       See below for specific examples

        To run all tests across this app:
            Test command: "tethys test -f tethys_apps.tethysapp.metdataexplorer2"

        To run all tests in this file:
            Test command: "tethys test -f tethys_apps.tethysapp.metdataexplorer2.tests.tests"

        To run tests in the Metdataexplorer2TestCase class:
            Test command: "tethys test -f tethys_apps.tethysapp.metdataexplorer2.tests.tests.Metdataexplorer2TestCase"

        To run only the test_if_tethys_platform_is_great function in the Metdataexplorer2TestCase class:
            Test command: "tethys test -f tethys_apps.tethysapp.metdataexplorer2.tests.tests.
            Metdataexplorer2TestCase.test_if_tethys_platform_is_great"
...
class Metdataexplorer2TestCase(TethysTestCase):
    """
    ...
    def set_up(self):
        ...
        test database creation function here, like so:
            ...
            self.create_test_persistent_stores_for_app(Metdataexplorer2)
    ...
    def tear_down(self):
        ...
        stores, you should call the test database destruction function from here, like so:
            ...
            self.destroy_test_persistent_stores_for_app(Metdataexplorer2)
    ...
    def test_home_controller(self):
        ...
        response = c.get('/apps/metdataexplorer2/')  # The final '/' is essential for all pages/controllers
